...
parser.add_argument('--model_name', default='ft_DesNet121', type=str, help='output model name')
parser.add_argument('--pretrained_model', default='https://download.pytorch.org/models/densenet121-a639ec97.pth',
                    type=str,
                    help='pretrained model path')
parser.add_argument('--data_dir', default='/home/gq123/guanqiao/deeplearning/reid/market/pytorch', type=str,
...
pretrained_model = "./densenet121-a639ec97.pth"
...
class dcganDataset(Dataset):
    def __init__(self, root, transform=None, targte_transform=None):
        ...
        self.image_dir = os.path.join(data_dir, root)
...
class LSROloss(nn.Module):
    def __init__(self, cuda=True):  # change target to range(0,750)
    ...
    def forward(self, input, target, flg):
        """
        input: the output of model, input.dim() = 2
        target: labels
        ...
        if self.cuda:
            flg = flg.type(torch.cuda.FloatTensor)  # for gpu
        else:
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloaders[phase]:
                ...
                running_corrects += torch.sum(preds == labels.data).item()  # update
            ...
            if phase == 'val':
                if epoch_acc > best_acc:
                    ...
                    best_model_wts = model.state_dict()
                if epoch >= 40:
                    save_network(model, epoch)  # save model
                    #    draw_curve(epoch)
        print()
...
if __name__ == '__main__':
    ...
    dataloaders = {}
    dataloaders['train'] = DataLoader(dcganDataset('train_new', data_transforms['train']), batch_size=batch_size,
                                      shuffle=True, num_workers=0)
    dataloaders['val'] = DataLoader(dcganDataset('val_new', data_transforms['val']), batch_size=batch_size,
                                    shuffle=True, num_workers=0)

    dataset_sizes = {}
    dataset_train_dir = os.path.join(data_dir, 'train_new')
    dataset_val_dir = os.path.join(data_dir, 'val_new')
    dataset_sizes['train'] = sum(
        len(os.listdir(os.path.join(dataset_train_dir, i))) for i in os.listdir(dataset_train_dir))
    ...
    if use_dense:
        ...
        model = ft_net_dense(pretrained_model, 751)  # 751 class for training data in market 1501 in total
    else:
        # checkpoint = torch.load("")
        model = ft_net(pretrained_model, 751)

    use_gpu = torch.cuda.is_available()
    if use_gpu:
        model = model.cuda()
    criterion = LSROloss(use_gpu)
