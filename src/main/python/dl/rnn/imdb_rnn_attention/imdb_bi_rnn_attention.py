...
version :
refer :
https://github.com/AllenCX/IMDB-RNN-Attention/blob/master/imdb-bi-rnn-attention.ipynb
"""
import sys

sys.path.append("E:/learning-to-rank/src/main/python/dl/rnn/imdb_rnn_attention/")
import numpy as np
import json
import tensorflow as tf

from dl.rnn.imdb_rnn_attention.config import Config
from dl.rnn.imdb_rnn_attention.utils import *
...
class imdb_classifier(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("attention") as scope:
            self._atn_in = tf.expand_dims(self.encoder_output,
                                          axis=2)  # [batch_size, max_time, 1, 2 * encoder_hidden_size]
    ...
    def test(self, sub_size=None, restore=False):
        ...
        loss_list = []
        for x_batch, y_batch, input_length in self.minibatches(
                self.x_train[:train_sub_size], self.y_train[:train_sub_size], self.train_length[:train_sub_size],
        ...
        loss_list = []
        for x_batch, y_batch, input_length in self.minibatches(
                self.x_test[:test_sub_size], self.y_test[:test_sub_size], self.test_lentgh[:test_sub_size],
                self.batch_size, False):
            x_batch, _ = self.padding_sequence(x_batch, self.max_length)
            feed_dict = {
                self.encoder_input: x_batch,
    ...
    def predict(self, inputs, restore=False):
        ...
        feed_dict = {
            ...
            self.encoder_input_length: inputs_length,
            self.output_keep_prob: 1.0
        }
        pred = self.sess.run()
